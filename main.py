...
from mixmancer.gui.theme import CustomTheme, CustomButton, CustomSlider, CustomLabel
...
class gui(tk.Tk):
    def __init__(self):
        ...
        # Image button
        self.image_button = CustomButton(self, text='Select Image', command=self.open_image_popup)
        self.image_button.grid(row=0, column=0, padx=10, pady=5)
        self.selected_image_label = CustomLabel(self, text='Selected Image:')
        self.selected_image_label.grid(row=0, column=1, padx=10, pady=5)

        # Music button
        self.music_button = CustomButton(self, text='Select Music', command=self.open_music_popup)
        self.music_button.grid(row=1, column=0, padx=10, pady=5)
        self.selected_music_label = CustomLabel(self, text='Selected Music:')
        self.selected_music_label.grid(row=1, column=1, padx=10, pady=5)

        # Music volume slider
        self.music_volume_label = CustomLabel(self, text='Music Volume:')
        self.music_volume_label.grid(row=2, column=0, padx=10, pady=5)
        self.music_volume_slider = CustomSlider(self, from_=0, to=1, orient='horizontal', command=self.update_volume)
        self.music_volume_slider.set(0.5)
        self.music_volume_slider.grid(row=2, column=1, padx=10, pady=5)

        # Sfx button
        self.sfx_button = CustomButton(self, text='Select Sfx', command=self.open_sfx_popup)
        self.sfx_button.grid(row=4, column=0, padx=10, pady=5)

        # Sfx volume slider
        self.sfx_volume_label = CustomLabel(self, text='Sfx Volume:')
        self.sfx_volume_label.grid(row=5, column=0, padx=10, pady=5)
        self.sfx_volume_slider = CustomSlider(self, from_=0, to=1, orient='horizontal', command=self.set_sfx_volume)
        self.sfx_volume_slider.set(0.5)
        self.sfx_volume_slider.grid(row=5, column=1, padx=10, pady=5)

        # Stop Sfx button
        self.sfx_button = CustomButton(self, text='Stop Sfx', command=self.stop_sfx_sounds)
